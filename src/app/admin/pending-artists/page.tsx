...
export default function PendingArtistsPage() {
  ...
  const runDiagnostics = async () => {
    const logs: string[] = [];
    const addLog = (msg: string) => logs.push(`${new Date().toISOString().split('T')[1].split('.')[0]} - ${msg}`);
    ...
    try {
      ...
      addLog(`Token in LocalStorage: ${localToken ? "✅ Present" : "❌ Missing"}`);
      ...
      try {
        ...
        addLog(`✅ Status Response Keys: ${Object.keys(status.data).join(", ")}`);
        if (status.data.data && status.data.data.stats) {
             addLog(`✅ Pending Count (nested): ${status.data.data.stats.pendingArtists}`);
      ...
      for (const endpoint of endpoints) {
        ...
        try {
          ...
          addLog(`✅ ${res.status} - ${isArray ? `Array(${data.length})` : "Not Array"}`);
  ...
  const fetchPendingArtists = async () => {
    ...
    try {
      ...
      console.log("🔵 Pending Artists Page - Full response:", JSON.stringify(response, null, 2));
      
      if (response.success) {
        const artistsList = Array.isArray(response.data) ? response.data : [];
        console.log("🔵 Pending Artists Page - Setting artists:", artistsList.length);
        
        if (artistsList.length > 0) {
          console.log("🔵 Pending Artists Page - Artists found:", artistsList.map((a: any) => ({
            id: a._id,
        ...
        if (artistsList.length === 0) {
          ...
          toast.success(`Found ${artistsList.length} pending artist${artistsList.length !== 1 ? 's' : ''}`);
        }
      } else {
        console.error("🔴 Pending Artists Page - Failed to fetch:", response.error);
        if (response.error) {
      ...
      const errorMsg = error.message || error.response?.data?.message || "Failed to fetch pending artists";
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = error.message || error.response?.data?.message || "Failed to approve artist";
      toast.error(errorMsg);
    } finally {
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      const errorMsg = error.message || error.response?.data?.message || "Failed to reject artist";
      toast.error(errorMsg);
    } finally {
      setProcessingId(null);
    }
  ...
  return (
    <AdminLayout>
      ...
      <Card className="mt-8 border-yellow-200 bg-yellow-50">
        <CardHeader>
          <CardTitle className="text-yellow-800 text-lg">Debug Info</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-sm text-yellow-700 space-y-2">
            <p><strong>Total Artists Found:</strong> {artists.length}</p>
            <p><strong>Backend Pending Count (Dashboard):</strong> {stats?.pendingArtists ?? "Unknown"}</p>
            <p><strong>Status Check:</strong> If you see artists here but they are not pending, the status filter might be wrong.</p>
            
            <div className="mt-4">
              <Button onClick={runDiagnostics} size="sm" variant="outline" className="mb-2">
                Run Diagnostics
              </Button>
            ...
            <details>
              <summary className="cursor-pointer font-medium hover:underline">View Raw Data (First Item)</summary>
              <pre className="mt-2 p-2 bg-black text-white rounded text-xs overflow-auto max-h-60">
                {artists.length > 0 ? JSON.stringify(artists[0], null, 2) : "No data"}
              </pre>
            </details>
          </div>
        </CardContent>
